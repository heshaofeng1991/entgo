...
type ChannelQuery struct {
	...
	withChannelCosts *ChannelCostQuery
...
// QueryChannelCosts chains the current query on the "channel_costs" edge.
func (cq *ChannelQuery) QueryChannelCosts() *ChannelCostQuery {
	query := &ChannelCostQuery{config: cq.config}
...
func (cq *ChannelQuery) QueryOrders() *OrderQuery {
...
func (cq *ChannelQuery) Clone() *ChannelQuery {
	...
	return &ChannelQuery{
		...
		withChannelCosts: cq.withChannelCosts.Clone(),
...
// WithChannelCosts tells the query-builder to eager-load the nodes that are connected to
// the "channel_costs" edge. The optional arguments are used to configure the query builder of the edge.
func (cq *ChannelQuery) WithChannelCosts(opts ...func(*ChannelCostQuery)) *ChannelQuery {
	query := &ChannelCostQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
	cq.withChannelCosts = query
	return cq
}

// WithOrders tells the query-builder to eager-load the nodes that are connected to
// the "orders" edge. The optional arguments are used to configure the query builder of the edge.
func (cq *ChannelQuery) WithOrders(opts ...func(*OrderQuery)) *ChannelQuery {
	query := &OrderQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (cq *ChannelQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Channel, error) {
	...
	_spec.Node.Schema = cq.schemaConfig.Channel
	...
	if query := cq.withChannelCosts; query != nil {
		if err := cq.loadChannelCosts(ctx, query, nodes,
			func(n *Channel) { n.Edges.ChannelCosts = []*ChannelCost{} },
			func(n *Channel, e *ChannelCost) { n.Edges.ChannelCosts = append(n.Edges.ChannelCosts, e) }); err != nil {
			return nil, err
		}
	}
	if query := cq.withOrders; query != nil {
		if err := cq.loadOrders(ctx, query, nodes,
			func(n *Channel) { n.Edges.Orders = []*Order{} },
			func(n *Channel, e *Order) { n.Edges.Orders = append(n.Edges.Orders, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *ChannelQuery) loadChannelCosts(ctx context.Context, query *ChannelCostQuery, nodes []*Channel, init func(*Channel), assign func(*Channel, *ChannelCost)) error {
	...
	query.Where(predicate.ChannelCost(func(s *sql.Selector) {
		s.Where(sql.InValues(channel.ChannelCostsColumn, fks...))
	}))
...
func (cq *ChannelQuery) loadOrders(ctx context.Context, query *OrderQuery, nodes []*Channel, init func(*Channel), assign func(*Channel, *Order)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Channel)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Order(func(s *sql.Selector) {
		s.Where(sql.InValues(channel.OrdersColumn, fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (cq *ChannelQuery) sqlQuery(ctx context.Context) *sql.Selector {
	...
	t1.Schema(cq.schemaConfig.Channel)
	ctx = internal.NewSchemaConfigContext(ctx, cq.schemaConfig)
...
func (cgb *ChannelGroupBy) sqlScan(ctx context.Context, v any) error {
...
func (cs *ChannelSelect) sqlScan(ctx context.Context, v any) error {
