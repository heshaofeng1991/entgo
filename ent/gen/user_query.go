...
type UserQuery struct {
	...
	withAnnouncements *AnnouncementsQuery
	withFKs           bool
...
func (uq *UserQuery) QueryAnnouncements() *AnnouncementsQuery {
	query := &AnnouncementsQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) WithAnnouncements(opts ...func(*AnnouncementsQuery)) *UserQuery {
	query := &AnnouncementsQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withAnnouncements; query != nil {
		if err := uq.loadAnnouncements(ctx, query, nodes,
			func(n *User) { n.Edges.Announcements = []*Announcements{} },
			func(n *User, e *Announcements) { n.Edges.Announcements = append(n.Edges.Announcements, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UserQuery) loadAnnouncements(ctx context.Context, query *AnnouncementsQuery, nodes []*User, init func(*User), assign func(*User, *Announcements)) error {
	...
	query.Where(predicate.Announcements(func(s *sql.Selector) {
		s.Where(sql.InValues(user.AnnouncementsColumn, fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "create_by" returned %v for node %v`, fk, n.ID)
		}
		assign(node, n)
	}
	return nil
