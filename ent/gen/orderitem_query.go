...
func (oiq *OrderItemQuery) QueryTenant() *TenantQuery {
	query := &TenantQuery{config: oiq.config}
...
func (oiq *OrderItemQuery) IDs(ctx context.Context) ([]int64, error) {
	var ids []int64
...
func (oiq *OrderItemQuery) Clone() *OrderItemQuery {
	...
	return &OrderItemQuery{
		...
		withTenant: oiq.withTenant.Clone(),
...
func (oiq *OrderItemQuery) WithTenant(opts ...func(*TenantQuery)) *OrderItemQuery {
	query := &TenantQuery{config: oiq.config}
	for _, opt := range opts {
		opt(query)
	}
	oiq.withTenant = query
	return oiq
}
...
func (oiq *OrderItemQuery) prepareQuery(ctx context.Context) error {
	...
	if orderitem.Policy == nil {
		return errors.New("gen: uninitialized orderitem.Policy (forgotten import gen/runtime?)")
	}
...
func (oiq *OrderItemQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*OrderItem, error) {
	...
	if oiq.withTenant != nil || oiq.withOrder != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := oiq.withTenant; query != nil {
		if err := oiq.loadTenant(ctx, query, nodes, nil,
			func(n *OrderItem, e *Tenant) { n.Edges.Tenant = e }); err != nil {
			return nil, err
		}
	}
...
func (oiq *OrderItemQuery) loadTenant(ctx context.Context, query *TenantQuery, nodes []*OrderItem, init func(*OrderItem), assign func(*OrderItem, *Tenant)) error {
	ids := make([]int64, 0, len(nodes))
	nodeids := make(map[int64][]*OrderItem)
	for i := range nodes {
		if nodes[i].order_item_tenant == nil {
			continue
		}
		fk := *nodes[i].order_item_tenant
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "order_item_tenant" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (oigb *OrderItemGroupBy) sqlScan(ctx context.Context, v any) error {
...
func (ois *OrderItemSelect) sqlScan(ctx context.Context, v any) error {
