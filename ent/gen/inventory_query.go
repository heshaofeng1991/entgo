...
func (iq *InventoryQuery) QueryTenant() *TenantQuery {
...
func (iq *InventoryQuery) QueryProducts() *ProductQuery {
...
func (iq *InventoryQuery) IDs(ctx context.Context) ([]int64, error) {
	var ids []int64
...
func (iq *InventoryQuery) WithTenant(opts ...func(*TenantQuery)) *InventoryQuery {
	query := &TenantQuery{config: iq.config}
...
func (iq *InventoryQuery) WithProducts(opts ...func(*ProductQuery)) *InventoryQuery {
	query := &ProductQuery{config: iq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (iq *InventoryQuery) prepareQuery(ctx context.Context) error {
	...
	if inventory.Policy == nil {
		return errors.New("gen: uninitialized inventory.Policy (forgotten import gen/runtime?)")
	}
	if err := inventory.Policy.EvalQuery(ctx, iq); err != nil {
		return err
	}
	return nil
}
...
func (iq *InventoryQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Inventory, error) {
	...
	if iq.withTenant != nil || iq.withProducts != nil {
		withFKs = true
	}
	if withFKs {
	...
	_spec.Node.Schema = iq.schemaConfig.Inventory
	...
	if query := iq.withTenant; query != nil {
		if err := iq.loadTenant(ctx, query, nodes, nil,
			func(n *Inventory, e *Tenant) { n.Edges.Tenant = e }); err != nil {
			return nil, err
		}
	}
	if query := iq.withProducts; query != nil {
		if err := iq.loadProducts(ctx, query, nodes, nil,
			func(n *Inventory, e *Product) { n.Edges.Products = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (iq *InventoryQuery) loadTenant(ctx context.Context, query *TenantQuery, nodes []*Inventory, init func(*Inventory), assign func(*Inventory, *Tenant)) error {
	ids := make([]int64, 0, len(nodes))
	nodeids := make(map[int64][]*Inventory)
	for i := range nodes {
		if nodes[i].inventory_tenant == nil {
			continue
		}
		fk := *nodes[i].inventory_tenant
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "inventory_tenant" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (iq *InventoryQuery) loadProducts(ctx context.Context, query *ProductQuery, nodes []*Inventory, init func(*Inventory), assign func(*Inventory, *Product)) error {
	ids := make([]int64, 0, len(nodes))
	nodeids := make(map[int64][]*Inventory)
	for i := range nodes {
...
func (iq *InventoryQuery) sqlQuery(ctx context.Context) *sql.Selector {
	...
	t1.Schema(iq.schemaConfig.Inventory)
	ctx = internal.NewSchemaConfigContext(ctx, iq.schemaConfig)
